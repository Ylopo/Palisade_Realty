...
/** Human-readable month label for logging, computed without a lib dependency. */
...
// Vercel cron: GET with Bearer CRON_SECRET (runs monthly, see vercel.json)
...
async function runEventsJob(offsetMonths: number) {
  console.log(`[events-research] Starting research for ${monthLabelForOffset(offsetMonths)}`)
  ...
  try {
    ...
    console.log(`[events-research] Done — ${result.postsCreated} posts created for ${result.monthLabel}`)
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family:Inter,sans-serif;max-width:600px;margin:0 auto;padding:32px 24px;background:#fff;">
        <div style="background:#58172a;padding:20px 24px;border-radius:8px 8px 0 0;">
        ...
        <div style="background:#f8f7f4;padding:24px;border-radius:0 0 8px 8px;border:1px solid #e5e3de;border-top:none;">
          ...
          <a href="${queueUrl}" style="display:inline-block;background:#eeca00;color:#1a1a1a;text-decoration:none;padding:12px 24px;border-radius:6px;font-size:14px;font-weight:600;">Open VA Queue →</a>
        </div>
      </div>`,
  })
}
