...
// ─── Types ────────────────────────────────────────────────────────────────────
// `@/sanity/queries` does not export a shared blogPost type in this codebase
// (see lib/publish-service.ts, which defines its own local subset for the
// same reason) — so this page defines its own minimal shape, using the
// `oneup*`-prefixed submission-id field naming already established there
// instead of the source repo's `blotato*` naming.
...
function sanityThumbUrl(ref: string) {
  return `https://cdn.sanity.io/images/qjhzi2t2/production/${ref.replace('image-', '').replace(/-(\w+)$/, '.$1')}`
}
...
// Scheduled posts are time-specific — show weekday + time so clustering on a given day is obvious.
...
export default function VAQueuePage() {
  ...
  // Scheduled posts get their own section, ordered by scheduled publish date (soonest first)
  // so it's clear what's booked for when — and easy to spot gaps or clustering on a date.
  const scheduled = posts
    .filter(p => p.workflowStatus === 'scheduled' && !deletedIds.has(p._id))
    .sort((a, b) => new Date(a.scheduledPublishAt ?? 0).getTime() - new Date(b.scheduledPublishAt ?? 0).getTime())
  const other   = posts.filter(p => !['media_pending', 'media_ready', 'scheduled'].includes(p.workflowStatus ?? '') && !deletedIds.has(p._id))
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f8f7f4', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 24px' }}>
        ...
        {/* Scheduled — ordered by publish date so it's clear what's booked for when */}
        {scheduled.length > 0 && (
          <Section title="Scheduled" count={scheduled.length} accent="#1d4ed8" bg="#dbeafe" subtitle="Ordered by publish date — soonest first. Check the dates to avoid gaps or clustering.">
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 16 }}>
              {scheduled.map(post => <PostCard key={post._id} post={post} secret={secret} onDelete={handleDelete} fhStatus={fhStatuses[post._id]} />)}
            </div>
          </Section>
        )}
      ...
      <Link href={`/admin/va-queue/${post._id}?secret=${secret}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block' }}>
        ...
        >
          <div style={{ position: 'relative', height: 140, background: hasThumb ? '#e2e8f0' : '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
            {hasThumb && post.coverImage?.asset
          ...
          <div style={{ padding: '14px 16px 16px' }}>
            ...
            <div style={{ fontSize: 12, color: '#94a3b8' }}>Created {post.publishedAt ? fmtDate(post.publishedAt) : '—'}</div>
            {status === 'scheduled' && post.scheduledPublishAt && (
              <div style={{
                display: 'flex', alignItems: 'center', gap: 5, marginTop: 8,
                fontSize: 12, fontWeight: 700, color: '#1d4ed8',
                background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 6, padding: '5px 8px',
              }}>
                📅 Scheduled for {fmtDateTime(post.scheduledPublishAt)}
              </div>
            )}
          </div>
        </div>
