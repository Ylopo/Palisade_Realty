...
export default async function BlogDashboardPage({ searchParams }: Props) {
  ...
  // raw prior-period count, so the combined total's "change" is a reach-weighted
  // average of each platform's own change% rather than a re-derived diff.
  const platformsList: OneUpPlatformAnalytics[] = platforms
    ? ([platforms.facebook, platforms.youtube, platforms.tiktok, platforms.instagram].filter(
        (p): p is OneUpPlatformAnalytics => p !== null
      ))
    : []
  const totalReach30 = platformsList.reduce((sum, p) => sum + p.reach, 0)
  const totalReachChange =
    totalReach30 === 0
      ? 0
      : Math.round(
          platformsList.reduce((sum, p) => sum + p.changePercent * p.reach, 0) / totalReach30
        )
    ...
    <>
      ...
      <div style={s.container}>
        ...
        <div style={s.headerWrap}>
          <div style={s.brandLabel}>HEDDA PARASHOS · PALISADE REALTY</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(360px, 1fr))', gap: 16 }}>
                <div style={s.chartWrap}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 8 }}>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 700, color: '#58172a' }}>{fmtNum(postsAllTime)}</div>
                  </div>
                  <CumulativeChart
                    series={cumPosts}
                    color="#58172a"
                    fillFrom="rgba(88,23,42,0.18)"
                    fillTo="rgba(88,23,42,0)"
                    yLabelSuffix=" posts"
                  />
                </div>
                <div style={s.chartWrap}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 8 }}>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 700, color: '#a88900' }}>
                  ...
                  {cumSessions.length > 1 ? (
                    <CumulativeChart
                      series={cumSessions}
                      color="#a88900"
                      fillFrom="rgba(238,202,0,0.22)"
                      fillTo="rgba(238,202,0,0)"
        ...
        <div style={s.section}>
          ...
          <div style={s.chartWrap}>
            ...
            <div style={s.chartLegend}>
              <LegendDot color="#58172a" label="Posts published" />
              <LegendDot color="#a88900" label="Peak week" />
        ...
        <div style={s.section}>
          ...
          <div style={s.networkGrid}>
            ...
            <NetworkCard
              ...
              name="YOUTUBE"
              source="ONEUP"
              available={platforms?.youtube != null}
              current={platforms?.youtube?.reach ?? 0}
              metricLabel="VIEWS · 30 DAYS"
              changePct={platforms?.youtube?.changePercent ?? null}
              trend={[]}
              topPostTitle={platforms?.youtube?.topPost?.title ?? null}
              topPostMeta={platforms?.youtube?.topPost ? `${fmtNum(platforms.youtube.topPost.reach)} views` : ''}
            />
            <NetworkCard
              icon="🎵"
              name="TIKTOK"
              source="ONEUP"
              available={platforms?.tiktok != null}
              current={platforms?.tiktok?.reach ?? 0}
              metricLabel="VIEWS · 30 DAYS"
              changePct={platforms?.tiktok?.changePercent ?? null}
              trend={[]}
              topPostTitle={platforms?.tiktok?.topPost?.title ?? null}
              topPostMeta={platforms?.tiktok?.topPost ? `${fmtNum(platforms.tiktok.topPost.reach)} views` : ''}
            />
            <NetworkCard
              icon="f"
              name="FACEBOOK"
              source="ONEUP"
              available={platforms?.facebook != null}
              current={platforms?.facebook?.reach ?? 0}
              metricLabel="REACH · 30 DAYS"
              changePct={platforms?.facebook?.changePercent ?? null}
              trend={[]}
              topPostTitle={platforms?.facebook?.topPost?.title ?? null}
              topPostMeta={platforms?.facebook?.topPost ? `${fmtNum(platforms.facebook.topPost.reach)} reach` : ''}
            />
          </div>
        </div>
      ...
      <div style={{ position: 'relative', height: BAR_AREA }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', height: '100%', gap: 6 }}>
          {weeks.map((w, i) => {
            ...
            const color = isPeak ? '#a88900' : w.count > 0 ? '#58172a' : '#e0ddd8'
            return (
              <div
                key={i}
                style={{
              ...
              >
                {isPeak && (
                  <div style={{ position: 'absolute', top: -20, color: '#a88900', fontSize: 12, fontWeight: 700 }}>
                    {w.count}
                  </div>
                )}
                <div
...
function Sparkline({ values }: { values: number[] }) {
  ...
  return (
    <svg width="100%" height={h} viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" style={{ marginTop: 16 }}>
      <polyline fill="none" stroke="#58172a" strokeWidth="1.5" points={points} opacity="0.8" />
    </svg>
  )
}
...
// Brand palette mirrors app/globals.css — burgundy replaces the source's navy,
// gold replaces the source's blue accent.
const BRAND = '#58172a'
