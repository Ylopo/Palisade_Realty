...
function ScoreRing({ score, size = 52 }: { score: number; size?: number }) {
  ...
  const color = score >= TOP_PICK_THRESHOLD ? '#16a34a' : score >= SCORE_THRESHOLD ? BRAND : '#94a3b8'
      ...
      {rows.map(([label, val, max]) => (
        <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
          ...
          <div style={{ flex: 1, background: '#e2e8f0', borderRadius: 99, height: 4 }}>
            <div style={{ width: `${(val / max) * 100}%`, background: BRAND, borderRadius: 99, height: 4 }} />
          </div>
...
function IdeaCard({ idea, secret, onAction }: {
  ...
  return (
    <div style={{
      ...
      borderLeft: `4px solid ${isTopPick ? '#16a34a' : BRAND}`,
      ...
      <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center', marginBottom: 8 }}>
            ...
            <span style={{ background: '#fdf2f4', color: BRAND, fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 99 }}>
          ...
          {/* Proven pattern — genericized performance context (no external company named) */}
          {(idea.renickLift || idea.renickTitle) && (
            <div style={{
              ...
              <div style={{ fontSize: 13, color: '#78350f', lineHeight: 1.5 }}>
                {idea.renickTitle
                  ? <>A similar post <em>&ldquo;{idea.renickTitle}&rdquo;</em>{idea.renickLift ? <> drove <strong>{idea.renickLift} more traffic</strong> for this audience after publishing.</> : <> performed well with this audience.</>}</>
                  : <>This format consistently performs well with this audience — a strong signal it will resonate again.</>
                }
              </div>
              {idea.renickPattern && (
          ...
          {state.phase === 'idle' && (
            <div style={{ display: 'flex', gap: 8 }}>
              <button
                ...
                style={{
                  padding: '8px 20px', background: BRAND, color: '#fff',
      ...
      <div style={{
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <button
            ...
            style={{
              padding: '7px 16px',
              background: generating ? '#f1f5f9' : BRAND,
          ...
          >
            {generating ? '⏳ Searching San Diego news…' : '↻ Generate More Ideas'}
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 0, maxWidth: 1120, margin: '0 auto', padding: '0 24px', alignItems: 'flex-start' }}>
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              background: selectedCategory === null ? '#fdf2f4' : 'transparent',
              cursor: 'pointer', textAlign: 'left',
            }}
          >
            <span style={{ fontSize: 13, fontWeight: selectedCategory === null ? 700 : 500, color: selectedCategory === null ? BRAND : '#374151' }}>
              All
            </span>
          ...
          {CATEGORIES.map(({ key, label }) => {
            ...
            return (
              ...
              >
                <span style={{ fontSize: 13, fontWeight: isSelected ? 700 : 400, color: isSelected ? BRAND : '#374151', flex: 1, lineHeight: 1.3 }}>
                  {label}
                </span>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 1, flexShrink: 0, marginLeft: 6 }}>
                  {pending > 0 && (
                    <span style={{ fontSize: 10, fontWeight: 700, background: '#fdf2f4', color: BRAND, borderRadius: 99, padding: '1px 6px' }}>
                      {pending}
                    </span>
                  )}
        ...
        <div style={{ flex: 1, minWidth: 0, paddingTop: 28, paddingBottom: 40 }}>
          ...
          {strongIdeas.length > 0 && (
            <section style={{ marginBottom: 32 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                <h2 style={{ fontSize: 12, fontWeight: 800, letterSpacing: '0.12em', textTransform: 'uppercase', color: BRAND, margin: 0 }}>
                  Strong Ideas
                </h2>
                <span style={{ fontSize: 12, color: '#c98a9a', fontWeight: 600 }}>Score {SCORE_THRESHOLD}–{TOP_PICK_THRESHOLD - 1}</span>
              </div>
