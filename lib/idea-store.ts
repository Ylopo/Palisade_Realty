 ...
 *
 * Redis keys:
 *   hps:idea:{id}              — JSON blob, 14-day TTL
 *   hps:ideas:queue            — sorted set: score → id (pending ideas only)
 *   hps:ideas:breaking         — list of breaking idea IDs (48h TTL each)
 *   hps:covered:topics         — set of slugified topic strings (no TTL)
 */

import { Redis } from '@upstash/redis'
import type { IdeaCandidate, IdeaStatus } from '@/lib/types'

const IDEA_TTL   = 14 * 24 * 60 * 60  // 14 days
const BREAKING_TTL = 48 * 60 * 60     // 48 hours
const QUEUE_KEY    = 'hps:ideas:queue'
const BREAKING_KEY = 'hps:ideas:breaking'
const COVERED_KEY  = 'hps:covered:topics'
...
function ideaKey(id: string) { return `hps:idea:${id}` }
...
export async function saveIdea(idea: IdeaCandidate): Promise<void> {
  ...
  if (idea.urgency === 'breaking') {
    await redis.set(`hps:idea:breaking:${idea.id}`, idea.id, { ex: BREAKING_TTL })
 ...
 * @param topic — slugified topic string, e.g. "flood-zone-coronado" or "closing-costs-san-diego"
 */
...
const PERFORMANCE_WEIGHTS_KEY = 'hps:strategy:performance-weights'
