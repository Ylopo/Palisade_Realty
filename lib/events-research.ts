 ...
 *
 * Searches for upcoming San Diego events, writes 1 roundup + up to 3 spotlights,
 * and saves them to Sanity as workflowStatus: 'media_pending' for the queue.
 */
...
async function fetchEventResults(month: string, year: string): Promise<RawResult[]> {
  ...
  const queries = [
    `San Diego events ${month} ${year}`,
    `Coronado events festivals ${month} ${year}`,
    `Downtown San Diego things to do ${month} ${year}`,
    `Point Loma North Park San Diego events concerts festivals ${month} ${year}`,
    `Carmel Valley Chula Vista Mission Valley San Diego events ${month} ${year}`,
    `San Diego outdoor festivals community ${month} ${year}`,
    `San Diego farmers market community events ${month} ${year}`,
    `San Diego family activities kids ${month} ${year}`,
  ]
...
async function extractEvents(
  ...
  const response = await anthropic.messages.create({
    ...
    messages: [
      {
        role: 'user',
        content: `You are a local events researcher for San Diego, California.

Below are search results for events happening in San Diego in ${month} ${year}. Extract all distinct events.

For each event return:
- name: event name
- dates: specific dates or pattern (e.g. "May 10–12", "Every Saturday in May", "May 3")
- location: neighborhood/city and venue name
...
Rules:
- Only include San Diego area events (Downtown San Diego, Carmel Valley, Mission Valley, Chula Vista, Point Loma, North Park, Coronado, and broader San Diego County). Drop anything outside this region.
...
async function writeSpotlightPost(
  ...
  const response = await anthropic.messages.create({
    ...
    messages: [
      {
        role: 'user',
        content: `You are Hedda Parashos, Owner of Palisade Realty, who has led a San Diego brokerage for over a decade. You care deeply about this community and love sharing what makes it a great place to live, raise a family, and put down roots.
...
Your voice: warm, local, genuinely excited — not a press release. You're a neighbor sharing something worth knowing about. Briefly weave in (1–2 sentences max) why events like this are part of what makes San Diego such a great place to live and own a home.

Return a JSON object with EXACTLY these fields:
{
  "title": "Engaging headline that names the event and month — not clickbait, just clear and inviting",
  "excerpt": "2–3 sentences. Names the event, dates, and why San Diego residents should know about it.",
  "metaTitle": "SEO title under 60 chars",
  "metaDescription": "120–160 chars for search preview",
  "body": "Full post in plain text. Use ## for h2 headings, ### for h3. Structure: intro paragraph → What Is It section → What to Expect section → Dates & Details section (date, time, location, cost if known) → Why It Matters to Our Community (1–2 sentences, Hedda's personal note) → closing. 350–500 words total."
}
...
Return ONLY valid JSON, no markdown fences.`,
  ...
  return {
    ...
    sourceUrl: event.url ?? `https://www.sandiego.org`,
...
async function writeRoundupPost(
  ...
  const response = await anthropic.messages.create({
    ...
    messages: [
      {
        role: 'user',
        content: `You are Hedda Parashos, Owner of Palisade Realty, who has led a San Diego brokerage for over a decade. You genuinely love this community and enjoy sharing what's going on.

Write a monthly community events roundup blog post for San Diego in ${month} ${year}.
...
Lightly weave in (1–2 sentences, at the end) that one of the things that makes San Diego such a great place to live is how much there always is to do — and that it's one of the reasons your clients love settling here.

Return a JSON object with EXACTLY these fields:
{
  "title": "What's Happening in San Diego in ${month} — Events, Festivals & Things To Do",
  "excerpt": "2–3 sentences overview of the month's highlights across San Diego.",
  "metaTitle": "San Diego Events ${month} ${year} | Things To Do Guide",
  "metaDescription": "120–160 chars — a preview of the month's best events",
  "body": "Full post in plain text. Use ## for h2 headings. Structure: brief intro paragraph → then organized sections by theme (Festivals & Big Events, Outdoor & Farmers Markets, Family & Community, etc.) — list events with their dates and locations under each section → closing paragraph with Hedda's note about community. 400–600 words. For each event, give 2–3 sentences of description. Use - for bullet-style items."
}
...
Return ONLY valid JSON, no markdown fences.`,
  ...
  return {
    ...
    metaDescription: data.metaDescription,
    body: markdownToPortableText(data.body),
    sourceUrl: 'https://www.sandiego.org',
    sourceTitle: `San Diego Events — ${month} ${year}`,
  }
}
...
export async function runEventsResearch(offsetMonths = 1): Promise<EventsResearchResult> {
  ...
  // 4. Save all posts to Sanity in parallel (media_pending → queue)
